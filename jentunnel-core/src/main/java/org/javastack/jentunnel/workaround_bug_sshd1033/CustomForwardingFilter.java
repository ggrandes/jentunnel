 ...
 * Release will be available in sshd-core:2.6.0+
 */
public class CustomForwardingFilter extends AbstractInnerCloseable
	...
	public static final Set<ClientChannelEvent> STATIC_IO_MSG_RECEIVED_EVENTS = Collections
			.unmodifiableSet(EnumSet.of(ClientChannelEvent.OPENED, ClientChannelEvent.CLOSED));
	...
	public CustomForwardingFilter(ConnectionService service) {
	...
	@Override
	public synchronized SshdSocketAddress startLocalPortForwarding(SshdSocketAddress local,
	...
	protected void unbindLocalForwarding(SshdSocketAddress local, SshdSocketAddress remote,
		...
		if ((bound != null) && (localAcceptor != null)) {
			...
			if (log.isDebugEnabled()) {
				log.debug("unbindLocalForwarding({} => {}) no mapping({}) or acceptor({})", local, remote,
						bound, localAcceptor);
			}
		}
	}
	...
	@Override
	public synchronized SshdSocketAddress startRemotePortForwarding(SshdSocketAddress remote,
		...
		try {
			...
			synchronized (remoteToLocal) {
				...
				if (prev != null) {
					throw new IOException("Multiple remote port forwarding bindings on port=" + port
							+ ": current=" + remote + ", previous=" + prev);
				}
	...
	protected void signalTearingDownExplicitTunnel(SshdSocketAddress boundAddress, boolean localForwarding,
			SshdSocketAddress remote) throws IOException {
		try {
	...
	protected void signalTearingDownExplicitTunnel(PortForwardingEventListener listener,
	...
	protected void signalTornDownExplicitTunnel(SshdSocketAddress boundAddress, boolean localForwarding,
			SshdSocketAddress remoteAddress, Throwable reason) throws IOException {
		try {
			invokePortEventListenerSignaller(l -> {
			...
			if (t instanceof RuntimeException) {
				...
				throw new IOException("Failed (" + t.getClass().getSimpleName() + ")"
						+ " to signal torn down explicit tunnel local=" + localForwarding + " on bound="
						+ boundAddress, t);
			}
		}
	}

	protected void signalTornDownExplicitTunnel(PortForwardingEventListener listener,
			SshdSocketAddress boundAddress, boolean localForwarding, SshdSocketAddress remoteAddress,
	...
	protected void signalEstablishedDynamicTunnel(SshdSocketAddress local, SshdSocketAddress boundAddress,
			Throwable reason) throws IOException {
		try {
			invokePortEventListenerSignaller(l -> {
			...
			if (t instanceof RuntimeException) {
				...
				throw new IOException("Failed (" + t.getClass().getSimpleName() + ")"
						+ " to signal establishing dynamic tunnel for local=" + local + " on bound="
						+ boundAddress, t);
			}
		}
	}
	...
	protected void signalEstablishingDynamicTunnel(PortForwardingEventListener listener,
			SshdSocketAddress local) throws IOException {
		if (listener == null) {
			return;
		}
	...
	protected void unbindDynamicForwarding(SshdSocketAddress local, SocksProxy proxy, InetSocketAddress bound)
			throws IOException {
	...
	protected void signalTearingDownDynamicTunnel(PortForwardingEventListener listener,
	...
	protected void signalTornDownDynamicTunnel(SshdSocketAddress address, Throwable reason)
			throws IOException {
		try {
			invokePortEventListenerSignaller(l -> {
	...
	protected void signalTornDownDynamicTunnel(PortForwardingEventListener listener,
			SshdSocketAddress address, Throwable reason) throws IOException {
		if (listener == null) {
			return;
		}
	...
	public synchronized SshdSocketAddress localPortForwardingRequested(SshdSocketAddress local)
		...
		try {
			if ((filter == null) || (!filter.canListen(local, session))) {
				if (log.isDebugEnabled()) {
					log.debug("localPortForwardingRequested(" + session + ")[" + local + "][haveFilter="
							+ (filter != null) + "] rejected");
				}
				return null;
			}
		} catch (Error e) {
			...
			if (log.isDebugEnabled()) {
				log.warn("localPortForwardingRequested(" + session + ")[" + local
		...
		try {
			...
			synchronized (localForwards) {
				...
				added = localForwards.add(new LocalForwardingEntry(result.getHostName(), local.getHostName(),
						result.getPort()));
			}
	...
	public synchronized void localPortForwardingCancelled(SshdSocketAddress local) throws IOException {
		...
		synchronized (localForwards) {
			entry = LocalForwardingEntry.findMatchingEntry(local.getHostName(), local.getPort(),
					localForwards);
			if (entry != null) {
	...
	protected void signalEstablishingExplicitTunnel(SshdSocketAddress local, SshdSocketAddress remote,
		...
		try {
			...
			if (t instanceof RuntimeException) {
				...
				throw new IOException("Failed (" + t.getClass().getSimpleName() + ")"
						+ " to signal establishing explicit tunnel for local=" + local + ", remote=" + remote
						+ ", localForwarding=" + localForwarding, t);
			}
		}
	}
	...
	protected void signalEstablishedExplicitTunnel(SshdSocketAddress local, SshdSocketAddress remote,
			...
			if (t instanceof RuntimeException) {
				...
				throw new IOException(
						"Failed (" + t.getClass().getSimpleName() + ")"
								+ " to signal established explicit tunnel for local=" + local + ", remote="
								+ remote + ", localForwarding=" + localForwarding + ", bound=" + boundAddress,
						t);
			}
		}
	}
	...
	protected void signalEstablishedExplicitTunnel(PortForwardingEventListener listener,
		...
		listener.establishedExplicitTunnel(getSession(), local, remote, localForwarding, boundAddress,
				reason);
	}

	protected void invokePortEventListenerSignaller(Invoker<PortForwardingEventListener, Void> invoker)
			throws Throwable {
		Throwable err = null;
		try {
	...
	protected void invokePortEventListenerSignallerListeners(
			...
			Invoker<PortForwardingEventListener, Void> invoker) throws Throwable {
	...
	protected synchronized Closeable getInnerCloseable() {
		return builder().parallel(toString(), dynamicLocal.values()).close(localAcceptor)
				.close(dynamicAcceptor).build();
	}
	...
	protected IoAcceptor createIoAcceptor(Factory<? extends IoHandler> handlerFactory) {
		Session session = getSession();
		FactoryManager manager = Objects.requireNonNull(session.getFactoryManager(), "No factory manager");
		IoServiceFactory factory = Objects.requireNonNull(manager.getIoServiceFactory(),
	...
	class StaticIoHandler implements IoHandler {
		...
		public void sessionCreated(IoSession session) throws Exception {
			...
			TcpipClientChannel.Type channelType = (remote == null) ? TcpipClientChannel.Type.Forwarded
			...
			if (channelType == TcpipClientChannel.Type.Forwarded) {
				...
				if (localEntry != null) {
					...
					log.warn(
							"sessionCreated({})[local={}, remote={}] cannot locate original local entry for accepted={}",
			...
			channel.open().addListener(future -> {
				...
				if (t != null) {
					log.warn("Failed ({}) to open channel for session={}: {}", t.getClass().getSimpleName(),
							session, t.getMessage());
					if (debugEnabled) {
						log.debug("sessionCreated(" + session + ") channel=" + channel
								+ " open failure details", t);
					}
					CustomForwardingFilter.this.service.unregisterChannel(channel);
					channel.close(false);
				}
			});
		}
		...
		public void sessionClosed(IoSession session) throws Exception {
			TcpipClientChannel channel = (TcpipClientChannel) session
					.removeAttribute(TcpipClientChannel.class);
			Throwable cause = (Throwable) session.removeAttribute(TcpipForwardingExceptionMarker.class);
			if (debugEnabled) {
				log.debug("sessionClosed({}) closing channel={} after {} messages - cause={}", session,
						channel, messagesCounter, (cause == null) ? null : cause.getClass().getSimpleName());
			}
			if (channel == null) {
				return;
			}
		...
		public void messageReceived(IoSession session, Readable message) throws Exception {
			...
			if (traceEnabled) {
				log.trace("messageReceived({}) channel={}, count={}, handle len={}", session, channel,
						totalMessages, message.available());
			}
			...
			if (traceEnabled) {
				log.trace("messageReceived({}) channel={} pend count={} after processing message", session,
						channel, pendCount);
			}
		}
		...
		public void exceptionCaught(IoSession session, Throwable cause) throws Exception {
			...
			log.warn("exceptionCaught({}) {}: {}", session, cause.getClass().getSimpleName(),
					cause.getMessage());
			if (debugEnabled) {
	...
	public List<Map.Entry<Integer, SshdSocketAddress>> getLocalForwardsBindings() {
		synchronized (localToRemote) {
			return localToRemote.isEmpty() ? Collections.emptyList()
					: localToRemote.entrySet().stream() // return an immutable clone to avoid 'setValue' calls
	...
	public List<Map.Entry<Integer, SshdSocketAddress>> getRemoteForwardsBindings() {
		synchronized (remoteToLocal) {
			return remoteToLocal.isEmpty() ? Collections.emptyList()
					: remoteToLocal.entrySet().stream() // return an immutable clone to avoid 'setValue' calls
														 // on a shared instance
							.map(e -> new SimpleImmutableEntry<>(e.getKey(), e.getValue()))
